...
func newTableFromParquet(datasetID string, tableID string, tableSource string, clusterKeys []string, partitionKey string, partitionRangeStart int64, partitionRangeEnd int64, partitionRangeInterval int64) {
	...
	/*if len(clusterKeys) > 0 {
		newTableMetadata = &bigquery.TableMetadata{
			...
			RangePartitioning: &bigquery.RangePartitioning{
				Field: partionKey,
				Range: &bigquery.RangePartitioningRange{
					Start:    partionRangeStart,
					End:      partionRangeEnd,
					Interval: partionRangeInterval,
				},
			},
	...
	/*for _, s := range newTableMetadata.Schema {
		infoLogger.Println("New Schema Value :", s.Name, s.Required, s.Type)
	}*/
